...
2. **Stage the runtime payload** the translated bodies reference (`baseDir` = `.throughline`):
   ...
   - `scripts/` → `<dir>/.throughline/scripts/`, **excluding `scripts/adapters/`** (build-time tooling) **and `*.test.mjs`** (dev-only tests, never referenced by a design-system skill's runtime path — and shipping them would drag the installer's own test file, which contains the bare token `CLAUDE_PLUGIN_ROOT`, into the user's tree). Only the runtime scripts a skill actually reads are staged.
